...
function Comment({comment, setToggle}) {
  ...
  return (
    <div className='mt-[30px] bg-[#f5f5f5] p-[10px]'>
       <div className='flex items-center'>
          <p className='font-[500] text-[18px] mr-5'>{comment.name}</p>
          <p className='text-[#878787] text-[14px] mt-[1px]'>{new Date(comment.date).toDateString()}</p>
          {comment.name === account.username && 
            
                <div onClick={() => deleteComment()} className='mx-[6px] mt-[-4px] p-[4px] rounded-[10px] ml-auto'>
